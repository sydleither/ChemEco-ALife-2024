...
def graph_generation_pointplot(df, save_dir):
    fig, ax = plt.subplots()
    sns.pointplot(data=df, x="graph_type", y="score", ax=ax)
...
def graph_generation_correlation(df, save_dir, gg_type1, gg_type2):
    df = pd.pivot(df, index="edge_weights", columns="graph_type", values="score")
    df = df.reset_index()
    fig, ax = plt.subplots()
    ...
    fig.patch.set_alpha(0.0)
    fig.tight_layout()
    fig.savefig(f"{save_dir}/correlation_{gg_type1}_{gg_type2}.png", bbox_inches="tight")
    plt.close()
...
def main(exp_name):
    ...
    graph_generation_correlation(df, save_dir, "random", "sbm")
