...
def multiple_constraint_experiment(exp_dir, constraint_combo, num_generations):
    ...
    for i,eval_func in enumerate(eval_funcs):
        exp_name = i
        experiment_config(configs_path, exp_dir, exp_name, eval_func, network_size=20, num_generations=num_generations, popsize=500)
...
def uniform_experiment(exp_dir):
    ...
    exp_name = 0
    experiment_config(configs_path, exp_dir, exp_name, eval_func, network_size=20, num_generations=0, popsize=10)
...
def generate_scripts_batch(overall_exp_name, exp_dirs, config_names):
    ...
    chunks = []
    for i in range(0, len(output), 249):
        chunks.append(output[i:i + 249])
...
if __name__ == "__main__":
    ...
    if experiment_name == "single":
        ...
        for constraint in get_properties_of_interest():
            ...
            all_config_names.append(multiple_constraint_experiment(exp_name, [constraint], 200))
        generate_scripts_batch(experiment_name, all_exp_names, all_config_names)
        exit()
    elif experiment_name == "double":
        constraint_combos = list(combinations(get_properties_of_interest(), 2))
        all_config_names = []
        all_exp_names = []
        for constraints in constraint_combos:
            exp_name = "".join([abbreviate_property_name(c_i, 1) for c_i in constraints])
            all_exp_names.append(exp_name)
            all_config_names.append(multiple_constraint_experiment(exp_name, constraints, 500))
        generate_scripts_batch(experiment_name, all_exp_names, all_config_names)
        exit()
    elif experiment_name == "triple":
        constraint_combos = list(combinations(get_properties_of_interest(), 3))
        all_config_names = []
        all_exp_names = []
        for constraints in constraint_combos:
            exp_name = "".join([abbreviate_property_name(c_i, 1) for c_i in constraints])
            all_exp_names.append(exp_name)
            all_config_names.append(multiple_constraint_experiment(exp_name, constraints, 1000))
        generate_scripts_batch(experiment_name, all_exp_names, all_config_names)
        exit()
